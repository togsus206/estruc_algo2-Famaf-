...
map_t remove_test(map_t map, char *person) {
    printf("\n\n");
    printf("REMOVE %s-------------------------------", person);
    printf("\n");
    string_t p = string_create(person);
    map = map_remove(map, p);
    string_destroy(p);
    map_dump(map, stdout);
    return map;
}
...
int main(void) {
    ...
    map = map_put(map, string_create("vruvru"), string_create("luciani"));
    map = map_put(map, string_create("cesar"), string_create("alvarez"));
    map = map_put(map, string_create("rodri"), string_create("ortiz"));
    map = map_put(map, string_create("eltu"), string_create("tor"));

    // string_dump(map_get(map, string_create("rodri")), stdout);
