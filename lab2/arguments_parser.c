...
static void ketopt_permute(
    char *argv[], int j, int n) { /* move argv[j] over n elements to the left */
    int k;
    char *p = argv[j];
...
parse_arguments_result parse_arguments(int argc, char *argv[]) {
    ...
    int c;
    while ((c = ketopt(&opt, argc, argv, 1, "isqcpr", longopts)) >= 0) {
        switch (c) {
            case 'i': {
