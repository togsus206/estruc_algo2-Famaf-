...
void process(int array[], unsigned int length, algorithm_t algorithm,
    ...
    const double elapsed =
        (double)(stop - start) * MILLISECONDS / CLOCKS_PER_SEC;
...
int main(int argc, char *argv[]) {
    extern unsigned int contador_swaps;
    extern unsigned int contador_comparaciones;
    ...
    for (unsigned int i = 0u; i < result.files_count; ++i) {
        ...
        if (result.do_stupid_sort) {
            process(array, length, STUPID_SORT, result);
        }
        if (result.do_not_sort) {
            process(array, length, DO_NOT_SORT, result);
        }
        printf("El número de swaps es %d, el número de comparaciones es %d\n",
               contador_swaps, contador_comparaciones);
        printf("\n\n");
    }
