...
void show_usage() {
    ...
    printf(
        "-f <files> Run algorithms on the all the files listed. Stdin is used "
        "by default\n");
    ...
    printf(
        "-t Enable testing (checks VALUE field on the first line of each input "
        "file)\n");
    printf("-w <arg> Force knapsack capacity to [arg]. ");
    printf(
        "Reads KNAPSACK field on first line of each input file by default\n\r");
...
// expected value.
void parse_first_line(FILE *file, weight_t *weight, value_t *expected,
                      bool *test) {
...
int run_file(const char *filename, bool do_backtracking, bool do_dynamic,
             bool show_items, bool test, weight_t weight) {
    FILE *file = NULL;
    int fails = 0;
    value_t expected = 0;
            ...
            for (unsigned int i = 0; i < array_length; i++) {
                if (selected[i]) {
                    ...
                    printf("%s:%u:%u\n", string_ref(item_id(item)),
                           item_value(item), item_weight(item));
                }
            }
            if (test) {
...
bool is_option(const char *str) { return (*str == '-' && strlen(str) == 2); }
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "f:w:sbdt")) != -1) {
        switch (c) {
            case 'b':
    ...
    for (unsigned int i = 0; i < files_count; i++) {
        ...
        fails = run_file(filenames[i], do_backtracking, do_dynamic, show_items,
                         test, weight);
        printf("DONE %s.\n\n", filenames[i] == NULL ? "stdin" : filenames[i]);
    }
    if (fails > 0 && test) {
